...
//==============================================================================
void Initial_data::set_Gaussian(Grid_data grid, const double w0, const double r0, Field &n_v, Field &s_v, Field &p_v, Field &q_v, Field &phi)
{
   if(M<1e-20){
      assert(grid.exc_i==0);
      {
      ...
      for (int i=grid.exc_i + 1; i<grid.nx-1; ++i) {
        double r = grid.r[i];
        double exparg = (r-r0)/w0;
        double bump = amp*exp(- (exparg*exparg));
        double valuephi = pow((r/w0),2.)*bump;
        if(valuephi>1e-6){
          phi.v[i] = valuephi;
...
//==============================================================================
void Initial_data::set_shift_symm_scalarized(Grid_data grid, Field &n_v, Field &s_v, Field &p_v, Field &q_v, Field &phi){

   assert(M>0);
   assert(fabs(grid.lexp)<1e-20);
   assert(fabs(grid.mu)<1e-20);
   assert(r_u>r_l);
   assert(grid.exc_i>0);
   assert(grid.r[grid.exc_i]<2.*M);
   assert(r_l>2.*M);
   ...
   for(int i = grid.exc_i; i<grid.nx-1; i++){
      double ls = grid.ls;
      double r= grid.r[i];
      double phi_decoup = pow(ls,2)*((8*M)/(3.*pow(r,3)) + 2/pow(r,2) + 2/(M*r));
      double Q_decoup = pow(ls,2)*((-8*M)/pow(r,4) - 4/pow(r,3) - 2/(M*pow(r,2)));
      double P_decoup  = -s_v.v[i]*Q_decoup;
