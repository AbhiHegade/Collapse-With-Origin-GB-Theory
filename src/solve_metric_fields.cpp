...
void Solve_metric_fields::solve_shift(const Grid_data grid,Field &s_v, const Field &p_v, const Field &q_v, const Field &phi_v){
  ...
  if(exc_i==0){
    ...
    // pavg = (p_v.v[i] + p_v.v[i+1])/2.;
    // qavg = (q_v.v[i] + q_v.v[i+1])/2.;
    savg = interp_4_c(-s_v.v[i], s_v.v[i-1], s_v.v[i], s_v.v[i+1], s_v.v[i+2]);
    pavg = interp_4_c(p_v.v[i], p_v.v[i-1], p_v.v[i], p_v.v[i+1], p_v.v[i+2]);
    qavg = interp_4_c(-q_v.v[i], q_v.v[i-1], q_v.v[i], q_v.v[i+1], q_v.v[i+2]);
    ...
    // }
    {
      ...
      r_Der_P_ip1 = (0. - p_v.v[i])/(2.*dr[i+1]);

      r_Der_Q_i = (q_v.v[i+1] - q_v.v[i-1])/(2.*dr[i]);
      r_Der_Q_ip1 = (0. - q_v.v[i])/(2.*dr[i+1]);
...
void Solve_metric_fields::solve_lapse(const Grid_data grid, Field &n_v, Field &s_v, const Field &p_v, const Field &q_v, const Field &phi_v){
  ...
  if(exc_i==0){
...

    {
      ...
      //cout<<"i = "<<i<<"; n_v.v[i+1] = "<<n_v.v[i+1]<<endl;
    }
    {
      int i = exc_i+1;
    ...
    n_v.v[nx-2] =  n_v.v[nx-3];
    n_v.v[nx-1] = n_v.v[nx-3] ;
